...
def display():
    ...
    n_samples = st.number_input("Total number of samples", min_value=100, max_value=len(x_train), value=10000, step=100, key="semi_input_n")
    ...
    if st.button("▶️ Start Training", key='semi_btn_3'):
        ...
        for i in range(n_iterations):
            ...
            # Dự đoán nhãn giả (pseudo-labels)
            ...
            num_labeled = len(st.session_state.x_train_part)
            num_unlabeled = len(st.session_state.x_test)
            num_new_labels = len(new_x_train)
            
            # Kiểm tra nếu có nhãn thật của tập test
            correct_preds = "N/A"
            incorrect_preds = "N/A"
            error_rate = "N/A"

            if hasattr(st.session_state, "y_test"):  
                true_labels = st.session_state.y_test[confident_mask]
                correct_preds = np.sum(true_labels == new_y_train)  # Số lượng nhãn đúng
                incorrect_preds = np.sum(true_labels != new_y_train)  # Số lượng nhãn sai
                error_rate = incorrect_preds / max(num_new_labels, 1)  # Tỷ lệ dự đoán sai
            ...
            # Lưu lại kết quả vào session_state
            st.session_state.training_history.append({
                "Iteration": i + 1,
                "Unlabeled Samples": num_unlabeled,
                "New Pseudo-Labels": num_new_labels,
                "Correct Labels": correct_preds,   # Thêm số lượng nhãn đúng
                "Incorrect Labels": incorrect_preds,  # Thêm số lượng nhãn sai
    ...
    if st.session_state.model is not None:
        ...
        st.write("### 📈 Kết quả toàn bộ quá trình huấn luyện")
        st.table(df_history)
...
def demo():
    ...
    canvas_result = st_canvas(
        ...
        key=f"canvas_{selected_model_name}" 
    )
    if st.button("Predict Drawn Digit", key='semi_btn_2'):
        if canvas_result.image_data is not None:
            st.session_state["drawn_image"] = canvas_result.image_data
            st.warning("⚠️ Predicting...")
            try: 
                ...
                img = np.expand_dims(img, axis=-1)  # Thêm kênh màu
...
def semisupervised():
    tab1, tab2,  tab3= st.tabs(["⚙️ Huấn luyện",  "Demo", "🔥Mlflow"])
    with tab1:
        display()
    with tab2:
        demo()
